...
Późniejsza współpraca z Agentem B dostarczyła odrębnego, niezależnie sprawdzonego benchmarku na 123 dokumentach Polish-PCC-dev 61--183. Historyczny wynik head-match bez singletonów wynosi $54,48 \pm 0,50$ dla CorefSeg-AE v2 (trzy seedy), 33,55 dla v1 i 73,96 dla CorPipe 26 base; dodatkowy exact-match v2 wynosi $53,34 \pm 0,43$ w skali 0--100. Modele CorefSeg należą do repozytorium B, a nie do linkera A. Tabela~\ref{tab:benchmark-b-pcc} przypina źródła, a tabela~\ref{tab:benchmark-b-loss} opisuje straty eksportu. Wynik korzysta ze złotej składni i pozycji zer oraz nie jest porównywalny z własnym pilotem A na złotych wzmiankach w oknach. Nie stanowi dowodu jakości na tekstach prawnych ani wyniku nowej inferencji po sanityzacji głów.
...
Niezależnie od tego pilota, cztery seedy ablacji R5/R6 Agenta B na dev60 nie potwierdziły stabilnego zysku DAE: różnica metryki zmieniała znak. Jest to negatywny wynik wykonanych konfiguracji na PCC, nie uniwersalne obalenie autokoderów. Nie uzasadnia kolejnego długiego treningu DAE bez nowej hipotezy, kontrolowanej ablacji, baseline'u i testu wolnego od wpływu tej optymalizacji.
...
PB2 również pozostaje nierozstrzygnięte. Na danych syntetycznych żaden autokoder nie zdominował kontroli, a segmentacja była bardziej zmienna. Zewnętrzny benchmark B potwierdza działanie CorefSeg-AE v2 na naturalnym PCC, ale nie zastępuje wspólnego porównania z linkerem A. Nie wykonano pełnych ablacji skip-connections, wagi rekonstrukcji, wymiaru latentnego ani kary przechodniości. Do odpowiedzi potrzebne są wspólne dane, zgodny zakres detekcji wzmianek oraz pomiar pełnego potoku, w tym ekstrakcji embeddingów.
...
W naturalnym pilocie A nie wykonano pretrainingu domenowego, pełnych ablacji A1--A7, pięciu seedów ani dokumentowego bootstrapu różnic. Odrębne trzy seedy v2 i cztery seedy ablacji DAE w repozytorium B nie uzupełniają tych braków w protokole A. Pomiar treningu głowic nie obejmuje jednorazowego kosztu kodowania HerBERT-em. t-SNE przedstawia jeden dokument syntetyczny i nie pozwala wnioskować o globalnej geometrii przestrzeni.
...
Pierwszeństwo ma ocena zamrożonych modeli na ręcznie zatwierdzonym teście prawnym; srebrne etykiety ani zgodność dwóch systemów nie są goldem. Dokumenty PCC-dev 61--183 pozostają historycznym benchmarkiem, nie testem dla nowej architektury. Dalsze wykonanie E1--E5 dla pięciu seedów na wspólnych danych wymaga uprzedniego określenia nowej hipotezy, baseline'u, kryterium sukcesu i niezależnego testu. Próg, checkpoint i hiperparametry należy dobrać wyłącznie na dev, z kontrolą identycznych i blisko zduplikowanych dokumentów między zbiorami. Po porównaniu należy osobno raportować rozrzut seedów i sparowany bootstrap dokumentowy; ablacje nie mogą korzystać z wyników testu do wyboru wariantu.
